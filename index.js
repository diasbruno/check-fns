...
const keepFirst = (a, b) => !b || !a.result ? a : b; // Given A and B, try to merge the results.
...
const object = o => (b, smsg, merge) => (merge = merge || keepFirst, Object.keys(o).reduce((acc, c) => merge(acc, o[c](b[c], smsg, merge)), {
  msg: smsg,
  result: true
...
const all = (...args) => (b, smsg, merge) => (merge = merge || keepFirst, args.reduce((acc, c) => merge(acc, c(b, smsg, merge)), {
  msg: smsg,
  result: true
})); // Given many test functions,
...
const or = (...args) => (b, smsg, merge) => (merge = merge || keepFirst, args.reduce((acc, c) => {
  if (acc && acc.result) return acc;
  const x = c(b, smsg, merge);
  if (!acc && !x.result || x.result) return x;
  return merge(acc, x);
}, null)); // Given a test function and a message,
...
const V = (f, s, m) => b => (m = m || keepFirst, m(f(b, s, m), null)); // A validate function that return at the first error.


const validate = (f, b, s) => V(f, s, keepFirst)(b); // A validate function that collects all errors.
